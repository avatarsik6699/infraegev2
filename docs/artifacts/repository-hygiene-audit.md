...
Реализация и актуальные результаты приёмки ведутся в [Change 107](../changes/107-frontend-design-system-consolidation.md).
...
План исправлений: [Change 107](../changes/107-frontend-design-system-consolidation.md).
...
Конкретный Backlog: [Change 107 — Frontend design-system consolidation](../changes/107-frontend-design-system-consolidation.md).
...
Сейчас реализуется только Change 107. Следующие пакеты ниже — последовательные будущие
changes; номера им назначает `/plan` после закрытия предыдущего. Не создавать несколько
...
| 3. Оставшаяся runtime/infra hygiene | P2/P3, следующий change после 107 | C02, оставшаяся часть C03, повторно проверенные C04, A01/A02. Проверять актуальный diff после DS, а не старые численные списки | Web/auxiliary states не изменили нужное поведение, живые tokens/mounts сохранены; узкий gate и cleanup allowlist |
