...
| Database | Application PostgreSQL 18.6, pinned multi-platform image; separate runtime/import/migration/backup roles, Alembic revision `120_01` and server-owned practice model/tooling. Existing lesson consumers use the server-owned bank (SPEC §3). Live production remains PG16 until explicit release transfer |
