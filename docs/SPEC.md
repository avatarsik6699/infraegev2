...
аналитический scope.
Аккаунты, новый сбор аналитики, Topic-связи, полнотекстовый поиск и другие функции за пределами текущих
...
| Sitemap | `/sitemap.xml` | Только canonical URL публичной главной, каталога тем, privacy и `published`-уроков; review/lab/404 не включаются |
...
визуальный профиль **infraege** сочетает тёплую бумажную поверхность, почти чёрный ink, один muted
уровень текста и сдержанный оранжевый акцент. Художественный источник публичного знака —
`docs/artifacts/references/infraege-mark.svg`; направления композиции и применения зафиксированы в
`docs/artifacts/references/base.jpg` и `docs/artifacts/references/main-page.png`. Знак состоит ровно
из трёх органических камней: малый верхний — orange, два нижних — ink. Wordmark, подпись и строка
преимуществ остаются живым доступным текстом, а не частью SVG.

Выбранная типографика не меняется: Alegreya используется для display и wordmark, Golos Text — для
чтения, controls и подписей, JetBrains Mono — только для кода, данных и формул. Иерархию создают
типографика, whitespace, выравнивание и тонкие structural borders; обычный текст не получает
декоративные цвета. Reference orange разрешён для знака и отдельных деталей иллюстрации; small
text, focus и functional underlines используют более тёмный contrast-safe orange token. Оба не
...
стрелки связывают этапы с карточками и теоретическими паттернами, а обратная петля показывает цикл
«теория → практика → теория». Все маршруты имеют longitudinal gradient, не пересекаются и остаются понятными без
анимации. Каталог материалов на главной не дублируется. Expanded identity использует Alegreya только для wordmark, а Golos Text — для подписи и
строки «просто • понятно • бесплатно»; разделитель identity/navigation остаётся едва заметным.
Недоступные разделы navigation не выглядят рабочими: это вторичный неинтерактивный текст без меток
состояния; account actions отсутствуют. Literal «72% курса» существует только внутри скрытой от
accessibility tree референсной иллюстрации и не является данными пользователя. Единая SVG-схема
сохраняет карточки, этапы и фоновые обозначения на всех ширинах: в desktop-композиции она занимает
больше пространства справа с заходом к центру и масштабируется одновременно по доступной ширине
колонки и высоте viewport. После перехода layout в одну колонку схема использует width-led sizing,
чтобы на промежуточных ширинах не появлялись лишние боковые поля; на narrow viewport она целиком
вписывается в доступную ширину без обрезки и горизонтального overflow. Page-local visual boundary
локально содержит только декоративный bleed SVG-фильтров и не расширяет document canvas на
пограничных ширинах. Desktop-сцена использует более широкое асимметричное поле координат: укрупнённые
карточки и обозначения разведены к краям, имеют разный вертикальный ритм и наклоны, но остаются внутри
authored viewBox. Проверенный raster luminance
mask вариант отклонён архитектором, поэтому
runtime-композиция использует только authored SVG. Общие SVG-примитивы отвечают за линии,
многотональное градиентное затухание и стрелки. Hero может добавить один низкоконтрастный page-local ambient SVG field
с инженерной сеткой на всю основную плоскость и затуханием к границам header/footer,
который визуально связывает текст и learning map, но не заходит в header/footer и не снижает читаемость. Ambient field остаётся
нейтральным и статичным; под authored-паттернами карты локальное мягкое ослабление сетки не даёт двум структурам
накапливать контраст. Направленное оранжевое движение принадлежит только learning map. После hydration только
декоративные overlay-слои могут непрерывно переносить явный градиентный импульс по уже понятным связям и рамкам; они останавливаются вне viewport,
в скрытой вкладке и при `prefers-reduced-motion`; SSR/no-JavaScript всегда отдаёт полную статичную композицию. Общие pattern
fields отвечают за направленные fade masks, а общий декларативный preset собирает локально заданные
штрихи, подписи и узлы без случайной генерации геометрии. Смысл паттерна, авторские контуры,
координаты и responsive-композиция остаются локальными у страницы. Общий custom-icon слой хранит нормализованные SVG viewport, accessibility mode и
семантические ink/paper/accent paint hooks для утверждённых авторских знаков, а размер, размещение,
текст и поверхность карточки остаются у consumer. Исходные контуры четырёх карточек главной
...
| `M6` — каталог тем ЕГЭ | in progress | Дать ученику правдивую полную карту экзамена до расширения TopicLesson-контента | `/ege` покрывает номера 1–27 через 25 учебных тем, объединяет 19–21, связывает только два опубликованных урока и оставляет остальные позиции явно запланированными |
...
| `22` | Вернуть public identity infraege и переработать главную по утверждённым референсам | Complete: Changes 95–97 вернули трёхкаменный знак и public metadata/chrome, переработали только `/`, ссылки/footer, глубину и restrained motion; typography и внутренние learning layouts остались прежними |
| `23` | Открыть полную карту тем ЕГЭ | Change 99: `/ege` показывает 25 тем по заданиям 1–27, объединяет 19–21 и ведёт только к двум действительно опубликованным TopicLesson |
