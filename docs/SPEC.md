...
| Course catalog | Публичная карта самостоятельных программ | Строится из lightweight publication metadata и трёх catalog-only planned записей; показывает один крупный опубликованный Python course и три меньших направления в редакционной мозаике, не загружает TSX-теорию, не добавляет фиктивные маршруты и не обещает сроки, длительность или объём будущих курсов |
...
и отдельная оранжевая SVG-траектория. Спокойная перспективная сетка и два фрагмента кода заменяют
координатные отметки и повторяющиеся фоновые импульсы. Python занимает семь колонок и две строки;
«Excel с нуля» и «Алгоритмы и структуры данных» — две карточки по пять колонок; «Решение задач
повышенной сложности» завершает композицию широкой карточкой с крупным лабиринтом.
На medium Python полноширинный, средние карточки образуют пару, последняя остаётся полноширинной;
narrow layout сохраняет порядок курсов в одной колонке. Все четыре supplied illustrations
доставляются оптимизированными WebP через shared Image и располагаются отдельно от текста.
Светлая бумага иллюстраций удаляется при отображении через page-owned SVG color-to-alpha filter,
с ограничением исходной альфой; рисунки частично выходят за верхний/правый край материальной
поверхности, не перекрывая текст и действия; края изображений плавно затухают. Бумажная палитра
общей дизайн-системы, тонкие нейтральные рамки, мягкие контактные тени и тихие гравировки создают
объём. Декор Python затухает выше footer: ссылка и прогресс читаются без отдельной подложки.
Траектория лестницы продолжается между карточками и нижним левым фрагментом,
а перспективная сетка остаётся видимой. Блики и световые маркеры activity-aware, проигрываются
