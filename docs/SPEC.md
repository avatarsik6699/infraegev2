...
этапы намеренно не соединены линиями: semantic trajectory будет добавлена отдельным решением. Каталог материалов на главной в этот этап не
дублируется. Expanded identity использует Alegreya только для wordmark, а Golos Text — для подписи и
...
градиентное затухание и стрелки, но learning map пока не использует их для связей. Общие pattern
