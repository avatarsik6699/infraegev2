...
состояния; account actions отсутствуют. Иллюстративное кольцо «72%» существует только внутри скрытой от
accessibility tree референсной иллюстрации и не является данными пользователя. Единая SVG-схема
сохраняет карточки, этапы и фоновые обозначения на всех ширинах. Desktop разделяет текст и карту
без наложений; карта на desktop ограничена доступной высотой экрана после header/footer, а короткие экраны
используют компактную типографику. Узкие/reflow-экраны сохраняют естественную прокрутку. На узких экранах та же SVG-сцена
использует укрупнённую вертикальную композицию с отдельной геометрией связей, выбранной CSS до
hydration. Декоративный bleed не расширяет document canvas. Авторские SVG остаются runtime-источником;
отклонённый raster mask не возвращается. Перспективная сетка занимает всю страницу; кодовые и математические паттерны распределены по обоим
краям, приглушены за читаемым текстом и затухают к header/footer. Тихие световые маршруты проходят
через общий фон в 22-секундном цикле. Заголовок «Информатика - это система»
сохранён как принцип проекта. Поясняющий абзац ведёт к одной ссылке со стрелкой «Начать готовиться» на `/ege`.
Поле затухает до нулевой видимости у границ header/footer, без резкого обрезания. Укрупнённые карточки имеют внутреннюю
гравировку; круговой прогресс — одну бумажную поверхность, тонкую шкалу и оранжевую дугу.
Внешние карточки компактны (282–290 × 108–116 SVG units), с плоской поверхностью и тонким
контуром, без теней, толщины и статического блика. 88-unit иконки находятся внутри границ,
с отступом 12 units слева и 12 units до текста. Карточки имеют близкие, немного разные размеры, без размытия и масштабирования по глубине.
Теория использует дуги, практика точки, задания небольшое переплетение петель, статистика плавные
контуры. Карточки и этапы имеют едва заметную волокнистую текстуру и приглушённые узоры. Круг прогресса
имеет чистую поверхность без декоративных паттернов и текстуры. Компактные номера (40 SVG units) и названия (24) находятся внутри границ этапов;
галочки (24) центрированы по вертикали. Шкала прогресса проходит
по границе бумажного круга, без выступающей подложки. В центре только крупное оранжевое «72%»,
без подписи «прогресс». После hydration общий 12-секундный цикл последовательно
ведёт импульс по основному пути (0–4 с), отвечает ветвями и бликами (4–7 с), затем выдерживает паузу.
Статические линии остаются полными; разная амплитуда desktop-дрейфа карточек (5–15 SVG units)
согласована с концами связей. Карточки основного маршрута, их текст и круг прогресса неподвижны.
Дрейфуют только внешние блоки с иконками; общий foreground-transform отсутствует. Фон сохраняет
тихое движение в 18-секундном цикле.
Эффекты приостанавливаются вне viewport и в скрытой вкладке без сброса фазы; reduced motion и
SSR/no-JavaScript сохраняют полную статичную композицию. Каталог тем сохраняет обычные карточки в одной ячейке сетки без растягивания на строки/колонки.
Все карточки имеют едва заметные текстуры и разные дуговые, точечные и диагональные узоры.
Все 25 карточек имеют равную высоту на каждой ширине экрана и общий шаблон: область изображения
на верхние 58% карточки, отдельное двухстрочное место под уменьшенное название, описание
и нижняя строка. Базовая высота — 23rem; полные исходные описания отображаются двумя компактными строками
с троеточием. Заголовки немного увеличены, отступы между заголовком, описанием и ссылкой — 8px. У тем без изображения — одинаковая нейтральная
заглушка с книгой. Номер и бейдж «Скоро» лежат рядом поверх изображения. Заголовки выровнены по нижнему краю двухстрочной области, описания
начинаются на общем уровне; затухание изображений ограничено полосой 8px у краёв. Иллюстрации тем 5/16 укрупнены за счёт
выступа за свою область на 12px сверху и 8px по бокам/снизу, без смещения текстовых зон. В выровненной нижней строке
расположена ссылка опубликованной темы. Вертикальные оранжевые маршруты убраны; стабильную сетку
дополняют восемь распределённых композиций (байты, AND, граф, бинарный поиск, таблица истинности,
рекурсия, степени двойки, BFS) и мягкое освещение в 24-секундном цикле, с паузой вне
видимости/в скрытой вкладке и без анимации при reduced motion/no-JS. Однократный блик опубликованных
карточек сохранён. Порядок 25 тем,
покрытие 1–27 и сетка 3/2/1 сохраняются. Общие pattern
...
Общий стандарт опирается на принятые `/courses` и `/courses/python`: инженерные сетки,
паттерны, бумажные поверхности, градиенты, глубина, иллюстрации и блики применимы ко всем новым
модулям, включая внутреннее оформление учебных блоков и рабочих форм. Декор не ухудшает
читаемость, фокус, ввод, семантическую обратную связь или SSR/no-JavaScript; reduced motion
сохраняет статическую композицию. Change 103 объединяет механизмы четырёх витрин и показывает
учебные варианты на стенде; опубликованные уроки сохраняют текущий вид до отдельной проработки. Навигация, порядок материала, domain/API/content/progress contracts и авторский
...
Каталог `/courses` использует общий с темами формат карточек с крупными иллюстрациями.
Title, lead и «1 курс доступен · 3 в плане» собраны слева; справа расположены supplied perspective
staircase и отдельная оранжевая SVG-траектория. Сохранена исходная асимметрия: Python занимает семь
колонок и две строки по 19rem, Сложные задачи и алгоритмы расположены справа в пяти колонках; нижняя широкая
карточка Excel (минимум 24rem) содержит текст слева и изображение на правых 55%.
Её бейдж и текст выровнены по левому отступу; текст начинается в 64px от верха, паттерны находятся слева. На medium Python полноширинный, затем
пара planned карточек и широкая нижняя; на узких экранах — одна колонка с минимумом 26rem.
Иллюстрации полностью заполняют отведённую область через cover с обрезкой периферии. Заголовки 32px,
описания до двух строк с троеточием и ссылка отделены от изображения и друг от друга отступами 8px.
Бейджи числа уроков, персонального прогресса и «Скоро» находятся на бумажных подложках поверх рисунка.
Все четыре supplied illustrations доставляются оптимизированными WebP через shared Image.
Для Python и сложных задач светлая бумага удаляется page-owned SVG color-to-alpha filter.
Excel и алгоритмы используют новые PNG-источники с готовой альфой, доставленные в WebP без
изменения альфы; фильтр удаления бумаги к ним не применяется;
выступ ограничен 8px сверху и по бокам, затухание краёв — узкой полосой 8px. Текст и действия не
перекрываются. Все поверхности имеют едва заметную зернистость и разнообразные дуговые, точечные
и диагональные паттерны. Сдержанный серебристо-нейтральный градиент, немного усиленная зернистость и прежние дуги у нижних
углов дополняют тонкие рамки. Мягкий блик материала проигрывается один раз за четыре секунды,
приостанавливается вне экрана и отключён при reduced motion.
Верхняя лестница приглушена и плавно затухает вниз. Оранжевые линии между Python и правой парой,
а также справа от этой пары удалены; осталась нижняя траектория к левому фрагменту.
Excel заполняет широкую область через cover с привязкой к верхнему краю и небольшой обрезкой
периферии; все изображения имеют мягкое затухание краёв шириной 8px; перспективная сетка остаётся видимой. Блики и световые маркеры activity-aware, проигрываются
однократно не дольше пяти секунд; переключателя анимации нет. Без JS и при reduced motion
композиция статична. Planned карточки помечены «Скоро», не интерактивны и имеют приглушённые
иллюстрации без блика рамки; hover lift относится только к Python.
Содержание и действия доступны в SSR/no-JS; только персональный
счётчик «Освоено N из 28 уроков» появляется после hydration.
Поиск, filters, sort, duration, difficulty, synthetic progress и recommendation banner отсутствуют.
...
| `24` | Открыть каталог самостоятельных мини-курсов | Change 100: `/courses` показывает опубликованный Python с hydration-only локальным счётчиком и три неинтерактивных направления «Скоро» в иллюстрированной responsive мозаике |
