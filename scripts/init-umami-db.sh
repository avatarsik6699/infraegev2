...
role_exists=$(
  "${compose[@]}" exec -T --interactive=false postgres psql -U "$POSTGRES_USER" -d postgres -tAc \
    "SELECT 1 FROM pg_roles WHERE rolname = 'umami'"
)
if [[ $role_exists != 1 ]]; then
  "${compose[@]}" exec -T --interactive=false postgres psql -U "$POSTGRES_USER" -d postgres \
    -v ON_ERROR_STOP=1 -v role_password="$UMAMI_DB_PASSWORD" <<'SQL'
CREATE ROLE umami LOGIN PASSWORD :'role_password';
SQL
fi
