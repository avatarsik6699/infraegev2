...
import { analyticsConsentStore } from "./analytics-consent";

export type ProductAnalyticsEvent =
  | { name: "course_opened"; properties: { course: string } }
...
export function reportProductEvent(event: ProductAnalyticsEvent): void {
  if (
    analyticsConsentStore.getSnapshot() !== "granted" ||
    !hasSafeProperties(event)
  )
    return;
