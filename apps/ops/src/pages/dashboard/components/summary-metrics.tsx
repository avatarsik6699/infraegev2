...
export const SummaryMetrics: React.FC<{
  ...
  const freshSources = sourceStatuses.filter(
    (item) => item.state === "fresh",
  ).length;
  return (
    <SimpleGrid cols={{ base: 2, sm: 3, lg: 9 }}>
      ...
      <MetricCard
        label="CPU"
        value={`${data.summary.cpu}%`}
        detail="warning 70%"
      />
      <MetricCard
        label="RAM"
        value={`${data.summary.memory}%`}
        detail="warning 75%"
      />
      <MetricCard
        label="DISK"
        value={`${data.summary.disk}%`}
        detail="warning 70%"
      />
      <MetricCard
        label="VISITS"
        value={String(data.summary.visits)}
        detail={range}
      />
      <MetricCard
        label="LIVE 30M"
      ...
      <MetricCard
        label="BANS"
        value={String(data.summary.activeBans)}
        detail="active"
      />
      <MetricCard
        label="SOURCES"
